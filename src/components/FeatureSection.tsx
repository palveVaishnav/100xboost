...
export default function FeatureSection() {
    return (
        <div className="grid place-content-center text-center bg-gradient-to-tl from-[#000] to-[#262724] px-24 pt-20">
            ...
            <AnimateView opacity={0} customduration={1}>
                <div className="px-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-10">
                {serviceItems.map((item, index) => (
                    <AnimateView opacity={0} delay={0.1 * index}
                        key={item.title}
                    >
                        <div className="grid text-left p-4 gap-4 hover:shadow-md hover:shadow-[#ffffff10] rounded-xl cursor-pointer transition-all ease-in-out duration-500">
                            <>
                                <Image
                                    src={item.image}
                                    alt="Feature image"
